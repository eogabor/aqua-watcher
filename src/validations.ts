...
export function validateArguments(armguments: any) {
  ...
  if (
    (process.argv[2] === "watch" && process.argv.length !== 4) ||
...
//TODO
function validateConfigJSON(jsonPath: string): string {
  let jsonErrors = "";
  ...
  if (
    ...
    jsonErrors += `Offline window START not provided or format not valid. Required format:{H:number;M:number}. Current value: ${JSON.stringify(
      config.offlineWindowStart
    )}\n`;
  }
  //check offline window end
  if (
    !config.offlineWindowEnd ||
    ...
    jsonErrors += `Offline window END not provided or format not valid. Required format:{H:number;M:number}. Current value: ${JSON.stringify(
      config.offlineWindowEnd
    )}\n`;
  }
