...
import { AquaLoggerConfig } from "./config.model.js";
...
async function mountDrive() {
  if (process.platform === "win32") {
    ...
    while (!folderExists) {
      ...
      if (fs.existsSync(config.sourceFolderPath)) {
        ...
        logger.warn(
          `Target folder not found, trying again at ${getWakeUpTime().toLocaleString()}.`
        );
  ...
  if (process.platform === "linux") {
    ...
    while (!driveMounted) {
      logger.info(
        `Check if source folder is mounted:  ${config.sourceFolderPath}`
      );
      ...
      if (mountedPath.length === 0) {
        logger.info(
          `Trying to mount: ${config.sourceFolderPath} at destination ${config.mountFolderPath}`
        );
        try {
          execSync(
          ...
          logger.warn(
            `Couldn't mount ${config.sourceFolderPath} on ${
              config.mountFolderPath
            }, trying again at ${getWakeUpTime().toLocaleString()}\n
            ${JSON.stringify(error)}`
          );
          await sleepUntil(getWakeUpTime());
          continue;
      ...
      if (mountedPath.length > 0 && mountedPath !== config.mountFolderPath) {
        logger.warn(
          `Drive is mounted at wrong target. Current target: ${mountedPath} and it should be at: ${config.mountFolderPath}`
        );
        logger.info(
          `Trying to mount: ${config.sourceFolderPath} at destination ${config.mountFolderPath}`
        );
  ...
  logger.error(
    "This app is only optimalized for windows and linux systems. Your system is: " +
      process.platform
  );
  process.exit(1);
}
...
async function watchLogFile(date: Date): Promise<Boolean> {
  ...
  if (!fs.existsSync(logFilePath)) {
    logger.warn(
      `Log file of ${dayString} doesn't exists yet, checking again at ${getWakeUpTime()}`
    );
    setTimeout(() => {
      establishConnection();
 ...
 */
function sleepUntil(wakeUpDate: Date) {
...
function getWakeUpTime(): Date {
  ...
  if (
    currHour >= config.offlineWindowStart.H &&
 ...
 */
async function getLastLogTime(): Promise<Date> {
  ...
  await axios
    ...
    .then((res) => {
      logger.http(
        `[GET]:${config.getLastLogDateURL} [STATUS]:${
          res.status
 ...
 */
async function postData(producedFrames: FrameRecord[]) {
  await axios
