...
async function establishConnection() {
  ...
  while (!folderExists) {
    ...
    if (fs.existsSync(process.argv[3])) {
      ...
      console.log(
        "Target folder not found, trying again in " + timeoutS + " seconds..."
      );
...
async function syncPastDays() {
  ...
  console.log(
    "The following days need to be synced: " +
...
function syncFile(filePath) {
  ...
  fileContents.split(endOfLineChar).forEach((line) => {
    let frame = parseFrame(line);
    if (!frame || frame.productId === "WMAN") {
      return;
    }
    let productIndex = producedFrames.findIndex(
      (record) => record.productId === frame.productId
    );

    if (productIndex !== -1) {
      producedFrames[productIndex].currentlyProduced = frame.currentlyProduced;
    } else {
      producedFrames.push(frame);
    }
  });
  sendFrameData(producedFrames);
}

//TODO to get last logged date from the server
...
function parseBuffer(buffer) {
  ...
  buffer
    ...
    .forEach(function (line) {
      ...
      if (productIndex !== -1) {
        producedFrames[productIndex].currentlyProduced =
          frame.currentlyProduced;
      } else {
        producedFrames.push(frame);
      }
    });
...
function sendFrameData(producedFrames) {
  ...
  if (dayIndex !== -1) {
    for (let i = 0; i < producedFrames.length; i++) {
      ...
      let frameIndex = day.frames.findIndex((frame) => {
        return (
          frame.productId === producedFrame.productId &&
          frame.itemNo === producedFrame.itemNo
        );
      });

      if (frameIndex !== -1) {
        days[dayIndex].frames[frameIndex].currentlyProduced =
          producedFrame.currentlyProduced;
      } else {
...
function parseFrame(line) {
  ...
  let frame = {
    timestamp: new Date(splittedLine[1].replace(/\"/g, "")),
    productId: splittedLine[2],
    itemNo: splittedLine[3],
    width: splittedLine[4].split("-")[0],
    height: splittedLine[4].split("-")[1],
    frameType: splittedLine[5],
    currentlyProduced: Number(splittedLine[7]) + 1,
    total: splittedLine[8],
  };
